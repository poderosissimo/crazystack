...
export default function FAQPage() {
  ...
  const faqItems = [
    {
      ...
      answer:
        "Para encomendar via WhatsApp, siga estes passos: 1) Envie uma mensagem para nosso número oficial. 2) Informe o código ou nome do produto desejado. 3) Especifique a quantidade. 4) Confirme seu endereço de entrega. 5) Escolha o método de pagamento. 6) Aguarde a confirmação do pedido. Nossa equipe estará disponível para esclarecer qualquer dúvida durante o processo.",
    },
    {
      category: "Prazos de Entrega",
      question: "Qual o prazo de entrega para minha região?",
      answer:
        "Os prazos de entrega variam de acordo com a sua localização: 1) Capitais: 3-5 dias úteis. 2) Outras cidades: 5-10 dias úteis. 3) Áreas remotas: até 15 dias úteis. Após a confirmação do pedido, você receberá um e-mail com uma estimativa mais precisa e um código de rastreamento. Lembre-se que eventos imprevistos podem afetar esses prazos, mas faremos o possível para mantê-lo informado sobre o status da sua entrega.",
    },
    {
      category: "Opções de Pagamento",
      question: "Quais são as formas de pagamento aceitas?",
      answer:
        "Oferecemos diversas opções de pagamento para sua conveniência: 1) Cartão de crédito (parcelamento em até 6x sem juros). 2) Boleto bancário (vencimento em 3 dias úteis). 3) Pix (aprovação instantânea). 4) PayPal. 5) Transferência bancária. Para pedidos internacionais, aceitamos pagamentos via PayPal e cartão de crédito internacional. Em caso de dúvidas sobre alguma forma de pagamento, não hesite em nos contatar.",
    },
    {
    ...
    },
    {
      ...
      answer:
        "Para prolongar a vida útil de suas bijuterias, siga estas dicas: 1) Evite contato com água, perfumes e produtos químicos. 2) Guarde as peças separadamente para evitar arranhões. 3) Limpe regularmente com um pano macio e seco. 4) Evite dormir ou fazer exercícios usando as bijuterias. 5) Aplique perfume ou creme antes de colocar as joias. 6) Guarde em local seco e arejado. 7) Use um sachê anti-oxidante na caixa de joias para prevenir o escurecimento das peças.",
    },
    {
      category: "Frete e Custos",
      question: "Como é calculado o frete e existe frete grátis?",
      answer:
        "O cálculo do frete é baseado no CEP de destino e no peso total do pedido. Oferecemos frete grátis para compras acima de R$ 200,00 para todo o Brasil. Para calcular o frete: 1) Adicione os produtos ao carrinho. 2) Informe seu CEP na página do carrinho. 3) Escolha entre as opções de envio disponíveis. Trabalhamos com diversas transportadoras para oferecer o melhor custo-benefício. Em promoções especiais, podemos oferecer frete grátis para compras de menor valor, fique atento às nossas redes sociais!",
    },
    {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
            ...
            {categories.map((category, index) => (
              <TabsTrigger key={index} value={category.name}>
                <span
                  ...
                  title={`${category.name} - Clique para filtrar`}
                >
                  {category.icon}
                  <span>{category.name}</span>
                  <span className="text-xs bg-gray-200 px-2 py-1 rounded-full">
                    {
                      faqItems.filter((faq) => faq.category === category.name)
                        .length
                    }
                  </span>
                </span>
              ...
              <AccordionContent>
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    alert(
                      "Pergunta e resposta copiadas para a área de transferência!",
                    );
                  }}
                >
                  Compartilhar
          ...
          {topFAQs.map((faq, index) => (
            <TabsContent key={index} value={`faq-${index}`}>
              ...
              <p className="text-sm text-gray-500 mt-2">
                {Math.floor(Math.random() * 1000)} pessoas acharam esta resposta
                útil
              </p>
            </TabsContent>
          ))}
      ...
      <Dialog>
        ...
        <DialogContent>
          ...
          <div className="grid gap-4 py-4">
            ...
            <Button
              onClick={() =>
                window.open("mailto:contato@suaempresa.com", "_blank")
              }
            >
              E-mail
            </Button>
      ...
      {showSuggestion && (
        <div
          className="fixed bottom-24 right-6 bg-white p-4 rounded-lg shadow-lg max-w-sm"
          role="alert"
        >
          <button
          ...
          <p className="text-sm mb-4">
            Que tal verificar estas perguntas populares:
          </p>
