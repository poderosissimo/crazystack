...
import { Star, ChevronDown, ChevronUp, Package, Tool, Shield, Clock, DollarSign, ThumbsUp, ThumbsDown } from 'lucide-react'
...
const services: Service[] = [
  {
    ...
    features: ["Rastreamento em tempo real", "Equipe especializada", "Garantia de pontualidade"]
  },
  {
    id: 2,
  ...
  },
  {
    ...
    features: ["Menor preço garantido", "Opção de agendamento flexível", "Descontos para mudanças recorrentes"]
  }
]
      ...
      <div className="space-y-2">
        {distribution.reverse().map((count, index) => (
          <div key={5 - index} className="flex items-center">
            ...
            <Progress value={(count / service.comments.length) * 100} className="h-2 w-full mx-2" />
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="space-y-6">
        {services.map((service) => (
          <Card key={service.id} className={`${selectedService === service.id ? 'border-primary shadow-lg' : ''} transition-all duration-300`}>
            <CardHeader>
              <CardTitle className="flex justify-between items-center">
                <span>{service.company}</span>
                <Badge variant={service.rating >= 4 ? "success" : service.rating >= 3 ? "warning" : "destructive"}>
                ...
                <TabsContent value="details">
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="font-bold text-2xl text-primary">{`R$ ${service.price.toFixed(2)}`}</span>
                      <TooltipProvider>
                        <Tooltip>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
                        ...
                        <span className="text-sm text-muted-foreground">({service.totalRatings} avaliações)</span>
                      </div>
                      <Dialog>
                        ...
                        <DialogContent>
                          <DialogHeader>
                            <DialogTitle>Avaliações para {service.company}</DialogTitle>
                          </DialogHeader>
            ...
            <CardFooter className="flex justify-between">
              <Button onClick={() => handleSelect(service.id)} variant={selectedService === service.id ? "secondary" : "default"}>
                {selectedService === service.id ? 'Selecionado' : 'Selecionar'}
              </Button>
              <Button variant="outline" onClick={() => handleExpand(service.id)}>
