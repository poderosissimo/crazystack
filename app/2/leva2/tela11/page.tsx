...
import { Heart, ChevronDown, ChevronUp, Filter, Search, Star, X, ShoppingCart, Instagram, Facebook } from 'lucide-react'
...
const products = Array.from({ length: 50 }, (_, i) => ({
  id: i + 1,
  name: `Bijuteria ${i + 1}`,
  type: ['Colar', 'Brinco', 'Anel', 'Pulseira', 'Tornozeleira', 'Tiara', 'Broche'][Math.floor(Math.random() * 7)],
  price: Math.floor(Math.random() * 200) + 10,
  material: ['Ouro', 'Prata', 'Bronze', 'Aço Inoxidável', 'Ródio', 'Cobre', 'Latão'][Math.floor(Math.random() * 7)],
  color: ['Dourado', 'Prateado', 'Rosa', 'Azul', 'Verde', 'Roxo', 'Multicolorido'][Math.floor(Math.random() * 7)],
...
export default function Component() {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {showFilters && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4 p-4 bg-gray-100 rounded-lg">
          <Select onValueChange={(value) => setSelectedType(value)}>
            ...
            <SelectContent>
              {['Colar', 'Brinco', 'Anel', 'Pulseira', 'Tornozeleira', 'Tiara', 'Broche'].map(type => (
          ...
          <Select onValueChange={(value) => setSelectedMaterial(value)}>
            ...
            <SelectContent>
              {['Ouro', 'Prata', 'Bronze', 'Aço Inoxidável', 'Ródio', 'Cobre', 'Latão'].map(material => (
          ...
          <Select onValueChange={(value) => setSelectedColor(value)}>
            ...
            <SelectContent>
              {['Dourado', 'Prateado', 'Rosa', 'Azul', 'Verde', 'Roxo', 'Multicolorido'].map(color => (
                <SelectItem key={color} value={color}>{color}</SelectItem>
              ))}
            </SelectContent>
          </Select>
          <div>
            <label className="text-sm font-medium">Faixa de Preço: R${priceRange[0]} - R${priceRange[1]}</label>
            <Slider
              min={0}
              max={200}
              step={1}
          ...
          <Button onClick={resetFilters} variant="outline" className="col-span-full">
            Limpar Filtros
          </Button>
        </div>
      )}
      ...
      <div className="flex justify-between items-center mb-4">
        <p className="text-sm text-gray-600">{sortedProducts.length} produtos encontrados</p>
      ...
      <div className={viewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4" : "space-y-4"}>
        {currentProducts.map((product) => (
          <Card key={product.id} className={viewMode === 'grid' ? "flex flex-col" : "flex flex-row"}>
            ...
            <CardContent className={viewMode === 'grid' ? "" : "w-2/3"}>
              ...
              <div className="flex items-center mt-2">
                <p className="font-bold text-lg">
                  {product.discount > 0 ? (
                    <>
                      <span className="line-through text-gray-500 mr-2">R${product.price}</span>
                      <span className="text-red-500">R${(product.price * (100 - product.discount) / 100).toFixed(2)}</span>
                    </>
                  ) : (
                    `R$${product.price}`
                  )}
            ...
            <CardFooter className={`flex justify-between ${viewMode === 'grid' ? "mt-auto" : ""}`}>
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button variant="outline" onClick={() => toggleFavorite(product.id)}>
                      <Heart className={`mr-2 h-4 w-4 ${favorites.includes(product.id) ? 'fill-red-500' : ''}`} />
                      {favorites.includes(product.id) ? 'Favoritado' : 'Favoritar'}
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent>
                    <p>{favorites.includes(product.id) ? 'Remover dos favoritos' : 'Adicionar aos favoritos'}</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
              <Dialog>
                ...
                <DialogContent className="sm:max-w-[425px]">
                  ...
                  <div className="grid gap-4 py-4">
                    ...
                    <p><strong>Cor:</strong> {selectedProduct?.color}</p>
                      ...
                      {Array.from({ length: 5 }).map((_, index) => (
                        <Star
                          ...
                          className={`inline-block h-4 w-4 ml-1 ${
                            index < Math.floor(parseFloat(selectedProduct?.rating || '0'))
                              ? 'text-yellow-400 fill-current'
                              : 'text-gray-300'
                          }`}
                        />
                    ...
                    {selectedProduct?.discount > 0 && (
                      <p><strong>Desconto:</strong> {selectedProduct?.discount}%</p>
                    )}
                  </div>
      ...
      <footer className="mt-12 py-6 border-t">
        <div className="flex justify-between items-center">
          <p>&copy; 2024 Catálogo de Bijuterias. Todos os direitos reservados.</p>
