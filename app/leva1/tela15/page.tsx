...
import { CalendarIcon, ClockIcon, DollarSignIcon, MapPinIcon, SearchIcon, TruckIcon, UserIcon } from 'lucide-react'
...
export default function OrderSummary() {
  ...
  const [statusFilter, setStatusFilter] = useState<EnumMoveStatus | 'ALL'>('ALL')
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setMoves(prevMoves => 
        prevMoves.map(move => ({
          ...move,
          status: Math.random() > 0.8 ? getNextStatus(move.status) : move.status
        }))
      )
    }, 5000)
    ...
    <Card className="w-full max-w-4xl mx-auto">
      ...
      <CardContent>
        <div className="flex space-x-4 mb-4">
          ...
          <Select value={statusFilter} onValueChange={(value) => setStatusFilter(value as EnumMoveStatus | 'ALL')}>
            ...
            <SelectContent>
              ...
              <SelectItem value={EnumMoveStatus.IN_PROGRESS}>Em Progresso</SelectItem>
              <SelectItem value={EnumMoveStatus.COMPLETED}>Concluído</SelectItem>
        ...
        <Table>
          ...
          <TableBody>
            {filteredMoves.sort((a, b) => {
              const order = [EnumMoveStatus.IN_PROGRESS, EnumMoveStatus.PENDING, EnumMoveStatus.COMPLETED, EnumMoveStatus.CANCELED]
