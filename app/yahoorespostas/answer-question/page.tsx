...
export default function QuestionDetailsPage() {
  ...
  const handleAnswerVote = (index: number, value: number) => {
    setAnswerVotes((prev) =>
      prev.map((votes, i) => (i === index ? votes + value : votes)),
    );
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <main>
        ...
        <p className="mb-4">
          I'm new to React and I'm trying to understand how to create a basic
          component. Can someone explain the process step by step?
        </p>
        ...
        <section className="mb-8">
          ...
          {answerVotes.map((votes, index) => (
            <div
              key={index}
              className={`mb-6 p-4 rounded-lg ${index === 0 ? "bg-green-50 border border-green-200" : "bg-gray-50"}`}
            >
              {index === 0 && (
              ...
              <div className="flex items-start mb-2">
                <Avatar className="h-8 w-8">
                  <AvatarImage
                    src={`/placeholder.svg?height=32&width=32&text=${index + 1}`}
                    alt={`@user${index + 1}`}
                  />
              ...
              <p className="mb-4">
                This is an example answer to the question. It provides a brief
                explanation and some code snippets if necessary.
              </p>
              <div className="flex items-center gap-4">
                <div className="flex items-center">
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => handleAnswerVote(index, 1)}
                  >
                    <ArrowBigUp className="h-6 w-6" />
                  </Button>
