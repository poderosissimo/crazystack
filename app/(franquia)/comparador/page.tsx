...
export default function BuscaComparacaoFranquiasDetalhada() {
  ...
  const franquiasFiltradas = franquias.filter(
    (franquia) =>
      ...
      (propriaOuTerceirizada.length === 0 ||
        propriaOuTerceirizada.includes(franquia.propriaOuTerceirizada)),
  );
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-4xl font-bold mb-8">
        Busca e Comparação de Franquias
      </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-8">
        ...
        <div>
          <label className="block text-sm font-medium mb-2">
            Investimento Máximo: R$ {investimentoMax.toLocaleString()}
          </label>
          <Slider
            min={0}
            max={2500000}
        ...
        <div>
          <label className="block text-sm font-medium mb-2">
            Tempo de Retorno Máximo: {tempoRetornoMax} meses
          </label>
          <Slider
            min={0}
            max={60}
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex gap-4">
            <div className="flex items-center">
              ...
              />
              <label htmlFor="propria" className="ml-2">
                Própria
              </label>
            </div>
            <div className="flex items-center">
              <Checkbox
                ...
                onCheckedChange={() =>
                  togglePropriaOuTerceirizada("Terceirizada")
                }
              />
              <label htmlFor="terceirizada" className="ml-2">
                Terceirizada
              </label>
            </div>
          </div>
        ...
        {franquiasFiltradas.map((franquia) => (
          ...
          >
            <CardHeader>
              <CardTitle className="flex justify-between items-center">
                ...
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => toggleExpanded(franquia.id)}
                >
            ...
            <CardContent>
              ...
              <p>
                Investimento: R$ {franquia.investimentoMinimo.toLocaleString()}{" "}
                - R$ {franquia.investimentoMaximo.toLocaleString()}
              </p>
              <p>
                Faturamento Médio Mensal: R${" "}
                {franquia.faturamentoMedioMensal.toLocaleString()}
              </p>
              <p>Tempo de Retorno: {franquia.tempoRetorno} meses</p>
              {expandedCards.includes(franquia.id) && (
                <>
                  <p>
                    Lucro Médio Mensal: R${" "}
                    {franquia.lucroMedioMensal.toLocaleString()}
                  </p>
                  <p>
                    Taxa de Franchising: R${" "}
                    {franquia.taxaFranchising.toLocaleString()}
                  </p>
                  ...
                  <p>
                    Funcionários Necessários: {franquia.funcionariosNecessarios}
                  </p>
              ...
              return (
                <Card key={franquia.id}>
                  ...
                  <CardContent>
                    ...
                    <p>
                      Investimento: R${" "}
                      {franquia.investimentoMinimo.toLocaleString()} - R${" "}
                      {franquia.investimentoMaximo.toLocaleString()}
                    </p>
                    <p>
                      Faturamento Médio Mensal: R${" "}
                      {franquia.faturamentoMedioMensal.toLocaleString()}
                    ...
                    <p>
                      ...
                      {franquia.funcionariosNecessarios}
                    </p>
                    <p>Operação: {franquia.propriaOuTerceirizada}</p>
                    <p>
                      ROI Estimado:{" "}
                      {(
                        ((franquia.lucroMedioMensal * 12) /
                          franquia.investimentoMinimo) *
                        100
                      ).toFixed(2)}
                      % ao ano
                    </p>
